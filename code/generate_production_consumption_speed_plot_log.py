...
def make_plot(height: int, width: int, loaded_experiments: dict, workers: List[int], colors,
    ...
    consumed = [loaded_experiments['consumption']['production_rate'] for _ in workers]

    fig.add_trace(go.Scatter(x=x_data, y=consumed,
                             legendrank=1,
                             showlegend=True,
                             name='\u03C1<sub>EAA</sub>',
                             mode='lines+markers',
                             line=dict(color='orange', width=2, dash='solid'),
                             marker=dict(size=15, color='orange', symbol='diamond-tall')))
    ...
    fig.update_layout(showlegend=True, height=height, width=width, title={
        ...
        'yanchor': 'top'}, plot_bgcolor='rgba(0,0,0,0)', margin=dict(r=2, t=2, l=65, b=35),  # bargap=0.5,
                      legend=dict(font=dict(size=20), itemwidth=30, borderwidth=1.35,
                                  yanchor="bottom", y=0.05, xanchor="left", x=0.70,
    ...
    fig.update_yaxes(title="samples/sec (log-scale)", linewidth=1.5, linecolor='black', showline=True, showgrid=True, gridwidth=0.00001,
                     ...
                     title_standoff=20, tickvals=[10**i for i in range(-1, 7)],
                     ticktext=[f'10<sup>{i}</sup>' for i in range(-1, 7)])
